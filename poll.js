...
function tryBuiltinCommand(msg) {
  ...
  if (/^(ping|alive\??|online\??|status\??)$/.test(b)) {
    ...
    sendReply(msg.from, msg.subject, `${NERVE_BOTNAME} online. skillVersion: 032, openclaw: ${ver}`);
    return true;
  }
  ...
  if (/\b(version|skill version|what version)\b/.test(b)) {
    let ver = 'unknown';
    try { ver = execSync(`PATH=${NODE_BIN}:$PATH openclaw --version`, { encoding: 'utf8', timeout: 5000 }).trim(); } catch {}
    sendReply(msg.from, msg.subject, `${NERVE_BOTNAME}: skillVersion 032, openclaw ${ver}`);
    return true;
  }
...
async function main() {
  ...
  post(`${NERVE_SERVER}/heartbeat`, { name: NERVE_BOTNAME, skillVersion: '032', version: main._oclawVersion }, { Authorization: `Bearer ${NERVE_TOKEN}` }).catch(() => {});
  ...
  // ====== AUTO-STATUS MESSAGE FILTER ======
  // Drop builtin auto-status messages (e.g. "botname: poll.js updated from server. Restarting poller now.")
  // These are generated by builtin commands and never require a reply.
  const STATUS_PATTERNS = [
    /poll\.js updated from server/i,
    /restarting poller now/i,
    /skillVersion:\s*\d+/i,
    /openclaw:\s*v?\d+\.\d+/i,
  ];
  const nonStatus = decryptedMessages.filter(m => {
    const body = (m.body || '').trim();
    if (STATUS_PATTERNS.some(re => re.test(body))) {
      console.log(`[${new Date().toISOString()}] Dropping auto-status from ${m.from}: "${body.substring(0, 60)}"`);
      return false;
    }
    return true;
  });
  ...
  const nonAck = nonStatus.filter(m => {
    const body = (m.body || '').trim();
